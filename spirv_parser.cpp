...
void Parser::parse(const Instruction &instruction)
{
	...
	case OpVariable:
	{
		...
		{
			ir.global_variables.push_back(id);
		}
		...
		if (variable_storage_is_aliased(var))
			ir.aliased_variables.push_back(var.self);
	...
	case OpSwitch:
	{
		...
		ir.multiselect_merge_targets.insert(current_block->next_block);
	...
	case OpSelectionMerge:
	{
		...
		ir.selection_merge_targets.insert(current_block->next_block);
	...
	case OpLoopMerge:
	{
		...
		ir.loop_blocks.insert(current_block->self);
		ir.loop_merge_targets.insert(current_block->merge_block);
		...
		if (current_block->continue_block != current_block->self)
			ir.continue_blocks.insert(current_block->continue_block);
